...
class SinglePointCrossover(CrossoverStrategy):
    def crossover(self, selected: List[ScheduleInfo], num_to_produce: int, rate: float) -> List[ScheduleInfo]:
        ...
        for i in range(0, len(parents), 2):
            ...
            if random.random() < rate:
                cut = random.randrange(1, n)  # точка разреза от 1 до n-1
...
class SimulatedBinaryCrossover(CrossoverStrategy):
